  ...
  return (
    <CrudPage<Promotion>
      ...
      validate={(f): Record<string, string> => {
        ...
        const val = Number(f.discountValue);

        if (!f.code) e.code = "Kode wajib diisi";
        if (!f.name) e.name = "Nama wajib diisi";

        if (!f.discountValue) {
          e.discountValue = "Nilai diskon wajib diisi";
        } else if (isNaN(val) || val <= 0) {
          e.discountValue = "Nilai diskon harus lebih dari 0";
        } else if (f.discountType === "PERCENTAGE" && val > 100) {
          // Pengecekan 100% hanya untuk tipe PERCENTAGE
          e.discountValue = "Nilai persentase tidak boleh lebih dari 100%";
        }

        if (f.minimumPurchase && Number(f.minimumPurchase) < 0) {
          e.minimumPurchase = "Minimal belanja tidak boleh negatif";
        }
        ...
        if (
          ...
          e.endDate = "Tanggal berakhir tidak boleh sebelum tanggal mulai";
        }
      ...
      toPayload={(f) => ({
        ...
        minimumPurchase: Number(f.minimumPurchase) || 0,
        ...
        <div className="flex items-start justify-between gap-2 rounded-xl border border-border-subtle dark:border-outline-variant/20 glass p-md shadow-card">
          <div>
            <div className="flex items-center gap-2">
              ...
              {!p.isActive && (
                <span className="chip chip-neutral font-bold">NONAKTIF</span>
              )}
            </div>
